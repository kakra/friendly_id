...
class SluggedModelTest < Test::Unit::TestCase
  ...
  context "A slugged model with default FriendlyId options" do
    ...
    context "when creating slugs with numeric basenames" do
      setup do
        Post.create! :title => "test post in 2009", :content => "numerical basename", :published => true
      end

      should "not fail on creating slugs for existing basenames" do
        assert_difference Post, :count, 1 do
          Post.create! :title => "test post in 2009", :content => "numerical basename", :published => true
        end
      end

      should "not fail on creating slugs for new basenames" do
        assert_difference Post, :count, 1 do
          Post.create! :title => "test post in 2010", :content => "numerical basename", :published => true
        end
      end

      should "create a new slug every time" do
        assert_difference Slug, :count, 3 do
          Post.create! :title => "test post in 2009", :content => "numerical basename", :published => true
          Post.create! :title => "test post in 2010", :content => "numerical basename", :published => true
          Post.create! :title => "test post in 2011", :content => "numerical basename", :published => true
        end
      end
    end
